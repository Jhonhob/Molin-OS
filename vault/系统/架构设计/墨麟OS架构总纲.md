...
> **TL;DR** 墨麟OS v3.0 是 5 Agent × 8 根目录的平坦 Vault 架构，记忆系统通过 Supermemory（语义检索）+ Obsidian（全文检索）双通道实现 L1-L4 四层记忆分层，数据流由 Cron 定时调度驱动，Agent 产出自动同步到对应目录。
...
| `Agents/<agent>/ 系统与配置/` | 环境、配置、工具链、技能图 |
...
- **2026-05-19**: 从旧 vault 8目录体系迁移合并到架构设计目录，作为系统层唯一架构总纲，统一系统架构、部署决策、记忆架构三份文档
