...
| `config/hermes-agent/cron_jobs.md` | 22 个 Cron 作业 | 调度参考 |
...
| 时间 | 公司 | Agent | 任务 |
|------|------|-------|------|
| 每天 05:00 | 紫灵 | 墨嗅 | 🆕 GitHub独立日扫 |
| 每天 05:30 | 紫灵 | 墨嗅 | 🆕 ProductHunt日报 |
| 每天 06:00 | 紫灵 | 墨嗅 | 每日情报抓取 |
| 每天 07:00 | 银月 | 墨排 | 内容发布调度 |
| 每天 08:00 | 宋玉 | 墨单 | 威客平台巡检 |
| 每天 08:00 | 梅凝 | 墨译 | 海外技术学习 |
...
## Obsidian Vault 结构 (v2.0 · 2026-05-22)

### 目录结构

```
墨麟系统空间/
├── 00_集团主脑/       ← CEO决策、全局架构、跨司协调、系统规范
├── 01_紫灵_情报雷达/  ← 情报采集、竞品跟踪、SEO关键词、市场机会
├── 02_元瑶_知识变现/  ← 课程体系、销售SOP、私域运营、用户增长
├── 03_银月_内容获客/  ← 内容策略、爆款库、SEO文章、账号运营
├── 04_梅凝_出海收汇/  ← 海外市场、本地化、支付合规、产品发版
├── 05_宋玉_产品孵化/  ← 在孵项目、技术选型、接单记录、冷启动
├── 06_玄骨_系统中枢/  ← 技术架构、运维日志、成本优化、系统进化
└── _模板库_/          ← 所有文档类型的标准模板
```

### 文档类型体系（7种）

| emoji | 类型 | 含日期？ | 更新方式 | Cron 写入规则 |
|-------|------|---------|---------|-------------|
| 🌲_图谱_ | 常青知识图谱 | ❌ | 追加修改 | 追加 `### YYYY-MM-DD 更新` 节 |
| 🔬_研究_ | 深度研究 | ❌ | 追加新节 | SCQA+MECE 框架 |
| 📋_SOP_ | 标准流程 | ❌ | 覆盖更新+修订日志 | 5W1H+SMART 验收标准 |
| 📊_报告_ | 周期报告 | ✅ 含月/周 | 每期新建 | 唯一可新建含日期文件 |
| 📈_滚动_ | 滚动日志 | ❌ | 顶部追加（新在上） | `## YYYY-MM-DD` 节头 |
| ⚡_实验_ | 实验记录 | ❌ | 每次实验追加 | HADI 循环结构 |
| 🔑_决策_ | 决策记录 | ❌ | 追加（旧不删） | ADR 格式 |

### 核心原则：常青优先

- 日报/同类内容 → **追加**到 📈_滚动_ 文件，禁止每天新建
- 发现可归纳知识 → **追加**到 🌲_图谱_ 或 🔬_研究_
- 周期数据报告 → 唯一可新建含日期文件（📊_报告_）
- 文档必须有 frontmatter（type + domain + updated），MOC 索引文件同步更新
